...
export function harness() {
  ...
  function client() {
    ...
    return async (
      path,
      { method = "GET", body, key, admin = false, headers = {} } = {},
    ) => {
      const r = await worker.fetch(
        new Request(`https://pixai.test/api/${path}`, {
          method,
          headers: {
            cookie,
...
export async function simulate(call, scenario = "approved") {
  const q = await call("quotes", {
    method: "POST",
  ...
  const r = await call("sandbox/confirm", { method: "POST", body, key });
  return { r, q, body, key };
}
