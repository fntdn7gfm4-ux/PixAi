export const TRANSITIONS = {
  ...
  AWAITING_PAYMENT: ["PROCESSING_PAYMENT", "PAYMENT_FAILED"],
  ...
  PIX_PROCESSING: ["PIX_SENT", "PIX_FAILED", "UNDER_REVIEW"],
  PIX_SENT: ["COMPLETED", "CHARGEBACK"],
  COMPLETED: ["REFUNDED", "CHARGEBACK"],
  PAYMENT_FAILED: [],
  PIX_FAILED: ["PIX_PROCESSING", "REFUNDED", "UNDER_REVIEW", "CHARGEBACK"],
  UNDER_REVIEW: [
    "PAYMENT_APPROVED",
    "PAYMENT_FAILED",
    "REFUNDED",
...
export function transition(from, to) {
  if (!TRANSITIONS[from]?.includes(to))
    throw Error(`Transição não permitida: ${from} → ${to}`);
  return to;
}
export function sandboxTimeline(scenario) {
  ...
  states.slice(1).forEach((s, i) => transition(states[i], s));
  return states;
}
