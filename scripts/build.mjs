...
const hosting=JSON.parse((await readFile('.openai/hosting.json','utf8')).replace(/^\uFEFF/,''));
await writeFile('dist/.openai/hosting.json',JSON.stringify(hosting,null,2));
await cp('drizzle','dist/.openai/drizzle',{recursive:true});
console.log('Build concluído: frontend + API + migrações. Pagamentos reais bloqueados.');
